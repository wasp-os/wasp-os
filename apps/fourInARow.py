...
class FourInARowApp():
  ...
  def _showStats(self):
    ...
    wwdraw.string('level %d  won %d%%' % (self.level.value, pc), 0, 200, width=_NPIXEL)

  def _place(self, x, y, colour):
    wwdraw.fill(x=_NBORDER + x*_NCELL + _NPAD, y=_NBORDER + (_NROW-y)*_NCELL + _NPAD, w=_NCELL - 2*_NPAD, h=_NCELL - 2*_NPAD, bg=colour)

  def foreground(self):
    # draw board from state
    wwdraw.fill()
    if self.screen == _INTRO:
      wwdraw.string('FOUR IN A ROW', 0, 30, width=_NPIXEL)
      wwdraw.string('Touch sets column', 0, 80, width=_NPIXEL)
      wwdraw.string('swipe down to play', 0, 110, width=_NPIXEL)
      self.level.draw()
      self._showStats()
    else:
      wwdraw.fill(x=0, y=_NBORDER+_NCELL, w=_NPIXEL, h=_NPIXEL-_NCELL, bg=_BLUE)
    ...
    if event[0] == wasp.EventType.DOWN:
      if self.screen == _INTRO:
        ...
        if _gameOver(self.bPLAY):
          wwdraw.string('YOU WIN!', 0, 0, width=_NPIXEL)
        ...
        if _gameOver(self.bCOMP):
          wwdraw.string('YOU LOSE!', 0, 0, width=_NPIXEL)
          self.screen = _GAMEOVER
          return
