...
class Mines_darkApp():
        ...
        if self.S != 1:
            if wasp.watch.rtc.get_uptime_ms()-self.L >= 500:
                if self.C == 1 and self.G[i * 8 + j] != -3 and self.G[i * 8 + j] < 9:
                    ...
                    if self.G[i * 8 + j] == -1:
                        self.G[i * 8 + j] = -3
                        self.draw.line(i*30+2,j*30+2,(i+1)*30-3,(j+1)*30-3,3,0x5801)
                        self.draw.line(i*30+2,(j+1)*30-3,(i+1)*30-3,j*30+2,3,0x5801)
                    elif self.G[i * 8 + j] == -3:
                        self.G[i * 8 + j] = -1
    ...
    def draw_end(self, defeat=0):
        self.S = 1
        self.draw.set_color(0x5801,0x1082) if defeat else self.draw.set_color(0x01C4, 0x1082)
